...
# --- DÉFINITION DES CODES ANSI POUR LA COULEUR ET LE STYLE ---
RESET = '\033[0m'
BOLD = '\033[1m'
BLUE = '\033[94m'
GREEN = '\033[92m'
...
# --- NOUVELLE STRUCTURE DE DONNÉES DU QUIZ PAR THÈME ---
quiz_data = {
...
# --- INITIALISATION DE L'ÉTAT DU QUIZ (scores) ---
quiz_scores = {
    1: None,
    2: None,
    3: None,
...
def clear_screen():
    """Efface la console pour une transition propre."""
    os.system('cls' if os.name == 'nt' else 'clear')

def draw_header(title, length=70):
    """Dessine un en-tête encadré et coloré."""
    line = HORIZONTAL * (length - 2)
    print(f"{CYAN}{TOP_LEFT}{line}{TOP_RIGHT}{RESET}")
    padding = (length - len(title) - 4) // 2
    print(f"{VERTICAL}{' ' * (padding - 1)}{BG_BLUE}{BOLD}{title}{RESET}{CYAN}{' ' * (length - len(title) - padding - 3)}{VERTICAL}{RESET}")
    print(f"{CYAN}{BOTTOM_LEFT}{line}{BOTTOM_RIGHT}{RESET}")

# --- FONCTION D'AFFICHAGE DE QUESTION (Mise à jour pour 1, 2, 3, 4) ---
def display_question(q, theme_number):
    """Affiche une question et ses options avec un style amélioré, en utilisant 1, 2, 3, 4."""
    clear_screen()
    theme_data = quiz_data['themes'][theme_number]
    total_questions = len(theme_data['questions'])
    
    # Calcul de l'index de question dans le thème (1 à 20)
    current_q_index_in_theme = (q['questionNumber'] - 1) % 20 + 1
    
    q_title = f"{theme_data['name']} | Question {current_q_index_in_theme}/{total_questions}"
    
    print(f"{MAGENTA}{BOLD}--- {q_title} ---{RESET}")
    print(f"{CYAN}{HORIZONTAL * 50}{RESET}")
    
    print(f"\n{BLUE}{BOLD}{q['question']}{RESET}\n")
    
    options = q['answerOptions']
    for i, option in enumerate(options):
        # Utiliser des chiffres pour les options (1, 2, 3, 4)
        print(f"  {YELLOW}{i + 1}){RESET} {option['text']}")
        
    print(f"\n{CYAN}{HORIZONTAL * 50}{RESET}")
    print(f"{BOLD}Entrez le numéro de votre réponse ({YELLOW}1, 2, 3, 4{RESET}{BOLD}) ou {RED}R{RESET}{BOLD} pour Retour Menu : {RESET}", end="")


# --- FONCTION DE VÉRIFICATION DE RÉPONSE (Mise à jour pour 1, 2, 3, 4) ---
def check_answer(q, user_input, theme_number):
    """Vérifie la réponse de l'utilisateur (chiffres 1-4) et affiche la correction."""
    ...
    try:
        user_index = int(user_input) - 1  # Convertit 1 -> 0, 2 -> 1, etc.
        options = q['answerOptions']
        
        correct_option = next((opt for opt in options if opt['isCorrect']), None)
        
        print(f"\n{MAGENTA}{BOLD}*** CORRECTION ***{RESET}")
        
        if user_index >= 0 and user_index < len(options):
            if options[user_index]['isCorrect']:
                print(f"{GREEN}{BOLD}✅ Félicitations ! Votre réponse est correcte. {RESET}")
                is_correct_answer = True
            else:
                print(f"{RED}{BOLD}❌ Réponse incorrecte.{RESET}")
                if correct_option:
                    # Trouve l'index de la bonne réponse pour l'affichage (1, 2, 3, 4)
                    correct_index = options.index(correct_option) + 1
                    print(f"La bonne réponse était : {GREEN}{BOLD}{correct_index}) {correct_option['text']}{RESET}")
        else:
            print(f"{RED}🤔 Choix non reconnu (doit être 1, 2, 3 ou 4). Voici la correction : {RESET}")
            if correct_option:
                correct_index = options.index(correct_option) + 1
                print(f"La bonne réponse est : {GREEN}{BOLD}{correct_index}) {correct_option['text']}{RESET}")
    
    except ValueError:
        print(f"{RED}❌ Entrée invalide. Veuillez entrer un chiffre (1, 2, 3, 4) ou R.{RESET}")
        correct_option = next((opt for opt in q['answerOptions'] if opt['isCorrect']), None)
        if correct_option:
            correct_index = q['answerOptions'].index(correct_option) + 1
            print(f"La bonne réponse est : {GREEN}{BOLD}{correct_index}) {correct_option['text']}{RESET}")
        
    
    if 'correction' in q:
        print(f"\n{CYAN}{BOLD}💡 Cours : {RESET}{q['correction']}") 
        
    print(f"{MAGENTA}{HORIZONTAL * 50}{RESET}")
    
    # Retourne le score (1 ou 0) ou None en cas d'abandon.
    return 1 if is_correct_answer else 0


# --- FONCTION D'EXÉCUTION DU QUIZ PAR THÈME (Mise à jour pour la boucle de vérification) ---
def run_quiz_theme(theme_number):
    """Exécute les questions pour un thème donné."""
    theme_data = quiz_data['themes'][theme_number]
    ...
    for q in questions:
        display_question(q, theme_number)
        
        # Saisie sécurisée pour les chiffres 1-4 ou R
        while True:
            try:
                user_answer = input().strip()
                if user_answer.upper() in ['1', '2', '3', '4', 'R']:
                    break
                else:
                    print(f"{RED}Veuillez n'entrer que 1, 2, 3, 4 ou R (pour retourner au menu).{RESET}")
            except EOFError:
                sys.exit(0) 
            except KeyboardInterrupt:
                sys.exit(0) 
        
        if user_answer.upper() == 'R':
            # Abandon de la session
            clear_screen()
            print(f"{RED}Session de révision du thème '{theme_name}' abandonnée. Retour au menu principal.{RESET}")
            time.sleep(1.5)
            return
        
        # Vérification et correction
        result = check_answer(q, user_answer, theme_number)
        
        # Si le résultat est 0 ou 1 (et non None), on ajoute au score
        if result is not None:
             score += result
            
        print(f"\n{CYAN}Appuyez sur {BOLD}ENTRÉE{RESET}{CYAN} pour passer à la question suivante...{RESET}", end="")
        input()
        
    # --- FIN DU THÈME : ÉCRAN DE RÉSULTAT ET SAUVEGARDE ---
    clear_screen()
    draw_header(f"RÉSULTAT : {theme_name}", length=70)

    global quiz_scores
    quiz_scores[theme_number] = f"{score}/{total_questions}"
    
    print(f"\n{MAGENTA}Félicitations ! Vous avez complété ce thème de révision. {RESET}")
    
    print(f"\n{CYAN}{BOLD}Votre score pour ce thème est de : {GREEN}{score}/{total_questions}{RESET}{CYAN}{BOLD} !{RESET}")
    
    print(f"\n{CYAN}{HORIZONTAL * 50}{RESET}")
    print(f"\n{YELLOW}Appuyez sur {BOLD}ENTRÉE{RESET}{YELLOW} pour revenir au menu principal.{RESET}", end="")
    input()


# --- FONCTION DU MENU PRINCIPAL (inchangée dans sa logique) ---
def display_main_menu():
    """Affiche le menu de sélection de thèmes avec les scores."""
    
    while True:
        clear_screen()
        draw_header(f"MENU PRINCIPAL - {quiz_data['title']}", length=70)
        
        # Affichage du score global (somme des scores complétés)
        total_questions = 0
        total_correct = 0
        
        print(f"\n{MAGENTA}{BOLD}--- PROGRESSION GLOBALE ---{RESET}")
        
        all_completed = True
        
        for num in quiz_data['themes'].keys():
            # Mise à jour du total de questions pour l'affichage global, même si le thème n'est pas complété
            total_questions += len(quiz_data['themes'][num]['questions']) 
            
            if quiz_scores[num]:
                correct, _ = map(int, quiz_scores[num].split('/'))
                total_correct += correct
            else:
                all_completed = False
        
        # Affichage synthétique du score cumulé
        print(f"  {CYAN}Score cumulé : {YELLOW}{BOLD}{total_correct}/{total_questions} {RESET}")
        
        if all_completed and total_questions > 0:
            print(f"  {GREEN}{BOLD}🎉 TOUS LES THÈMES COMPLÉTÉS ! {RESET}")
        
        print(f"\n{MAGENTA}{BOLD}--- SÉLECTION DE THÈME ---{RESET}")
        
        # Affichage des options de thèmes avec le statut
        for num, theme in quiz_data['themes'].items():
            status_text = f"Non fait"
            status_color = YELLOW
            
            if quiz_scores[num]:
                status_color = GREEN
                status_text = f"Complété ({quiz_scores[num]})"
            
            padding = 50 - len(theme['name'])
            
            print(f"  {BLUE}{BOLD}{num}){RESET} {theme['name']}{' ' * padding} {status_color}[{status_text}]{RESET}")
            
        print(f"\n{CYAN}{HORIZONTAL * 70}{RESET}")
        
        # Choix de l'utilisateur
        print(f"{BOLD}Entrez le numéro du thème ({BLUE}1 à 5{RESET}{BOLD}) ou {RED}Q{RESET}{BOLD} pour Quitter : {RESET}", end="")

        user_choice = input().strip().upper()
        
        if user_choice == 'Q':
            print(f"\n{RED}Fermeture du Quiz. Au revoir !{RESET}")
            time.sleep(1)
            sys.exit(0)
        
        try:
            choice_number = int(user_choice)
            if choice_number in quiz_data['themes']:
                run_quiz_theme(choice_number)
            else:
                print(f"\n{RED}Sélection invalide. Veuillez choisir un numéro entre 1 et 5.{RESET}")
                time.sleep(1.5)
        except ValueError:
            print(f"\n{RED}Veuillez entrer un chiffre ou la lettre Q.{RESET}")
            time.sleep(1.5)
...
if __name__ == "__main__":
    try:
        ...
        print(f"{RED}Une erreur inattendue est survenue : {e}{RESET}")
        input(f"\n{YELLOW}Appuyez sur Entrée pour fermer.{RESET}")
        clear_screen()